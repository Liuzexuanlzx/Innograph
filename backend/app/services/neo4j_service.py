...
class Neo4jService:
    ...
    async def upsert_papers_batch(self, papers: list[Paper]) -> None:
        if not papers:
            return
        async with self._driver.session() as session:
            for paper in papers:
                if paper.openalex_id:
                    await session.run(
                        ...
                        SET p.doi = $doi,
                            ...
                            p.url = $url
                        """,
                        **paper.model_dump(exclude={"embedding"}),
                    )

    async def upsert_paper_cards_batch(self, cards: list[PaperCard]) -> None:
        if not cards:
            return
        async with self._driver.session() as session:
            for card in cards:
    ...
    async def upsert_innovation_edges_batch(self, edges: list[InnovationEdge]) -> list[str]:
        if not edges:
            return []
        edge_ids = []
        async with self._driver.session() as session:
